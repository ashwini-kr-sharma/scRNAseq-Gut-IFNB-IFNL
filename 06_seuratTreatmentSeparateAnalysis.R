...
saveRDS(object = seu, file = paste0(path, "analysis/postFilter_merge_counts/all_SeuratObjects_Combined.RDS"))
...
p_ileum = p_ileum_denovo / p_ileum_predict / p_ileum_ISG

ggsave(filename = paste0(path, "results/merged_after_kb/TreatmentSeparate/Ileum_celltypes.pdf"), plot = p_ileum, width = 9, height = 6)

rm(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p_ileum_denovo, p_ileum_predict, p_ileum_ISG)
...
p_colon = p_colon_denovo / p_colon_predict / p_colon_ISG

ggsave(filename = paste0(path, "results/merged_after_kb/TreatmentSeparate/Colon_celltypes.pdf"), plot = p_colon, width = 9, height = 6)

rm(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p_colon_denovo, p_colon_predict, p_colon_ISG)
...
#-------------------------------------------------------------------------------
# ISG fraction expressed
#-------------------------------------------------------------------------------

isg_exp = lapply(seu, function(x) {
  
  isgdat = as.matrix(x$SeuratObject@assays$RNA@data)
  isgdat = isgdat[rownames(isgdat) %in% isg2,]
  
  a = round((colSums(isgdat > 0)/nrow(isgdat))*100, 2)
  b = apply(isgdat, 2, function(x) mean(x[x>0]))
  
  df = data.frame(isg.per.exp = a, avg.exp = b, cells = x$SeuratObject$predicted.id)
  df = split(df[,1:2], df$cells)
  df = sapply(df, function(x) apply(x, 2, median))
  
  return(df)
})

for(i in names(isg_exp))
{
  isg_exp[[i]] = data.frame(Cells = colnames(isg_exp[[i]]), 
                            Treatment = i, 
                            t(isg_exp[[i]]))
}
rm(i)

isg_exp = do.call("rbind", isg_exp)

isg_exp_ileum = isg_exp[grepl("Ileum", isg_exp$Treatment),]
isg_exp_colon = isg_exp[grepl("Colon", isg_exp$Treatment),]

######
#Ileum
######
isg_exp_ileum = ggplot(isg_exp_ileum, aes(y = Treatment, x = Cells, color = avg.exp, size = isg.per.exp)) + theme_bw(base_size = 8) + 
                      labs(x = "", y ="", subtitle = "Ileum", color = "log2 mean expression", size = "% of ISGs expressed") +
                      geom_point() + scale_colour_gradient(low="pink", high="blue", 
                                                           limits = c(0, ceiling(max(isg_exp_ileum$avg.exp))), 
...
ggsave(filename = paste0(path, "results/merged_after_kb/TreatmentSeparate/Ileum_celltypes_fraction_ISG_expressed.pdf"), plot = isg_exp_ileum, width = 4, height = 3)

######
#Colon
######
isg_exp_colon = ggplot(isg_exp_colon, aes(y = Treatment, x = Cells, color = avg.exp, size = isg.per.exp)) + theme_bw(base_size = 8) + 
  labs(x = "", y ="", subtitle = "Colon", color = "log2 mean expression", size = "% of ISGs expressed") +
  geom_point() + scale_colour_gradient(low="pink", high="blue", 
                                       limits = c(0, ceiling(max(isg_exp_colon$avg.exp))), 
                                       guide = guide_colourbar(title.position = "top", barheight = 0.5, ticks = F)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1), legend.position = "bottom") +
  guides(size = guide_legend(title.position = "top"))
ggsave(filename = paste0(path, "results/merged_after_kb/TreatmentSeparate/Colon_celltypes_fraction_ISG_expressed.pdf"), plot = isg_exp_colon, width = 4, height = 3)

rm(isg_exp, isg_exp_colon, isg_exp_ileum)
...
# Plotting number of genes expressed for each cluster

# p =ggplot(seu$`Ileum|Mock`$SeuratObject@meta.data, aes(x = seurat_clusters, y = nFeature_RNA)) +
#   labs(x="", y="Number of genes expressed") + theme_bw(base_size = 9) +
#   #geom_violin(trim = T, color = "black", fill = "#1b9e77", draw_quantiles = 0.5, lwd=0.3) +
#   geom_boxplot(outlier.size = 0.1, lwd=0.3) +
# 
#   theme(panel.grid = element_blank(),
